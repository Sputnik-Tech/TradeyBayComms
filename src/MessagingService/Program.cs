...
using MessagingService.GrpcClients;
...
// Register the Auth gRPC clients. Ensure that your configuration contains the appropriate URLs.
builder.Services.AddGrpcClient<UserProfileService.UserProfileServiceClient>(options =>
{
...
builder.Services.AddScoped<IAuthUserProfileGrpcClient, AuthUserProfileGrpcClient>();

builder.Services.AddGrpcClient<BusinessService.BusinessServiceClient>(options =>
{
    var authServiceUrl = builder.Configuration["AuthServiceUrl"] 
                         ?? throw new InvalidOperationException("AuthServiceUrl is not configured");
    options.Address = new Uri(authServiceUrl);
}).AddInterceptor<AuthTokenInterceptor>();
builder.Services.AddScoped<IAuthBusinessGrpcClient, AuthBusinessGrpcClient>();

builder.Services.AddGrpcClient<StandardAdService.StandardAdServiceClient>(options =>
{
    // ** Use correct configuration key from appsettings.json **
    var adsServiceUrl = builder.Configuration["GrpcServices:StandardAdsServiceUrl"]
                         ?? throw new InvalidOperationException("GrpcServices:StandardAdsServiceUrl is not configured");
    options.Address = new Uri(adsServiceUrl);
}).AddInterceptor<AuthTokenInterceptor>(); // Add interceptor to propagate token
builder.Services.AddScoped<IStandardAdsGrpcClient, StandardAdsGrpcClient>(); // Register interface and implementation



builder.Services.AddScoped<AuthUserProfileMapper>();
builder.Services.AddScoped<AuthBusinessProfileMapper>();
builder.Services.AddScoped<StandardAdMapper>();
